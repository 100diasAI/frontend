...
const ProductDetail = ({theme}) => {
  ...
  const checkStock = async (cantidad = 1) => {
    ...
    const product = await axios.get(
      `https://backend-xsy3.onrender.com/product/${productId}`
    );
    if (talle === "Sin talle") {
...
/*   const handleVirtualTryOn = useCallback(() => {
  ...
  const isVirtualTryOnEligible = () => {
    const eligibleCategories = ['Remeras', 'Abrigo', 'Camisas'];
    return eligibleCategories.includes(product.categorium?.nombre);
  };
  return (
    <Main>
      {
      ...
      <Div>
        ...
        <InfoContainer>
          ...
          {/* Modificamos esta parte para mostrar el botón condicionalmente */}
          {isVirtualTryOnEligible() && (
            <Link 
              to="/vestidor" 
              state={{ productImage: product.imagen, productName: product.nombre, productId: productId }}
            >
              <Button>Probador Virtual</Button>
            </Link>
          )}
        </InfoContainer>
      </Div>
